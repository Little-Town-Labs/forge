...
import { Plus, Edit, Trash2, TestTube, Eye, EyeOff, AlertCircle, CheckCircle, Shield } from "lucide-react";
...
const AIModelConfig: React.FC = () => {
  ...
  const validateForm = (): boolean => {
    ...
    if (!formData.apiKey.trim()) {
      ...
      const apiKeyValidation = validationRules.apiKey(formData.provider);
      if (!apiKeyValidation.test(formData.apiKey)) {
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 border border-gray-200 dark:border-gray-600">
          <div className="flex items-center justify-between mb-4">
            ...
            {/* Security Badge */}
            <div className="flex items-center space-x-1 px-3 py-1 bg-blue-50 dark:bg-blue-900/20 rounded-full">
              <Shield className="w-4 h-4 text-blue-600 dark:text-blue-400" />
              <span className="text-sm font-medium text-blue-700 dark:text-blue-300">Secure Configuration</span>
            </div>
          </div>
          ...
          <div className="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
            <div className="flex items-start space-x-2">
              ...
              <div>
                <h4 className="font-medium text-amber-800 dark:text-amber-200">Security Best Practices</h4>
                <ul className="mt-1 text-sm text-amber-700 dark:text-amber-300 list-disc list-inside space-y-1">
                  ...
                  <li>Only use API keys from trusted sources and keep them confidential</li>
                  <li>Regularly rotate your API keys for enhanced security</li>
                  <li>Monitor usage through your provider&apos;s dashboard</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <ValidationInput
                label={`${formData.provider === 'openai' ? 'OpenAI' : 'Google AI'} API Key`}
                type="password"
                value={formData.apiKey}
                onChange={(value) => setFormData(prev => ({ ...prev, apiKey: value }))}
                placeholder={`Enter your ${formData.provider === 'openai' ? 'OpenAI' : 'Google AI'} API key...`}
                required={true}
                validationRules={[
                  validationRules.required("API key is required"),
                  validationRules.apiKey(formData.provider, `Please enter a valid ${formData.provider === 'openai' ? 'OpenAI' : 'Google AI'} API key`)
                ]}
                helperText={
                  formData.provider === 'openai' 
                    ? "OpenAI API keys start with 'sk-' and are stored encrypted"
                    : "Google AI API keys are stored encrypted and secured"
                }
                maxLength={200}
              />
              <div className="flex items-center space-x-1 text-xs text-gray-500 dark:text-gray-400">
                <Shield className="w-3 h-3" />
                <span>API keys are encrypted at rest and never logged in plaintext</span>
              </div>
            </div>
                        ...
                        <div className="flex items-center space-x-1">
                          <span className="font-mono text-sm text-gray-900 dark:text-white">
                            {showApiKey[model.id] 
                              ? (model.apiKey 
                                  ? (model.provider === 'openai' 
                                      ? `${model.apiKey.substring(0, 7)}...${model.apiKey.slice(-4)}`
                                      : `${model.apiKey.substring(0, 8)}...${model.apiKey.slice(-6)}`)
                                  : '[Not Set]')
                          ...
                          {model.apiKey && (
                            <button
                              ...
                              title={showApiKey[model.id] ? "Hide API key" : "Show partial API key"}
                            >
